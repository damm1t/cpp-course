...
void big_integer::mul(const uint32_t val)
{
	uint64_t tmp = 0;
...
void big_integer::add(const uint32_t val) // PRE : value  >= 0
{
	uint64_t tmp = val;
	for (size_t i = 0; i < data_.size(); ++i)
	{
...
uint32_t big_integer::div(const uint32_t val)
{
	uint32_t mod = 0;
...
{
	data_.clear();
	data_.push_back(static_cast<uint32_t>(abs(int64_t(val))));
